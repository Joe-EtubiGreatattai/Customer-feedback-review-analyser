...
def fetch_reviews():
    ...
    if not product_name:
        messagebox.showerror("Error", "Please enter the name of the product.")
        return
    ...
    try:
        ...
        review_div_tags = driver.find_elements(By.CSS_SELECTOR, 'article.-pvs.-hr._bet')

        review_list = []
        for div_tag in review_div_tags:
            header = div_tag.find_element(By.CSS_SELECTOR, 'h3.-m.-fs16.-pvs').text.strip()
            text = div_tag.find_element(By.CSS_SELECTOR, 'p.-pvs').text.strip()
            review_list.append({
                'header': header,
                'text': text
            })
        ...
        for i, review in enumerate(review_list, start=1):
            ...
            reviews_text.insert(tk.END, review['header'] + "\n")
...
window = tk.Tk()
window.title("Jumia Review Scraper")
...
reviews_text = tk.Text(window, height=30, width=100)
reviews_text.pack()
